...
class MultiSwitch (EventMixin):
  ...
  def start_monitoring(self):
    if self.previous_ping_returned == True:
      self.send_ping()
    else:
  ...
  def start_diagnosis(self):
	...
	msg.match.dl_dst = EthAddr(con.eth_addr)
      ...
      con.send(msg)
    self.send_ping()
    self.send_ping(eth_dst=EthAddr(core.openflow.getConnection(1).eth_addr))

    Timer(5, self.diagnosis_result, recurring=True)      

  def diagnosis_result(self):
    if(len(self.ping_replies) == 2):
      #print self.ping_replies.items()
      #print self.ping_replies['1']
      if(self.ping_replies[2] and self.ping_replies[3]):
        log.info("Link between S2 and S3 has failed")
      if(not (self.ping_replies[2]) and self.ping_replies[3]):
        log.info("Link between S1 and S2 has failed")
      if(self.ping_replies[2] and not (self.ping_replies[3])):
