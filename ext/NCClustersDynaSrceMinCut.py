...
*The cluster center with the highest number of nodes becomes the source node of the complete graph.
  ...
  def initialization(self):
    ...
    for x in range(0,self.num_of_nodes):  
      if x == self.SourceNode:
        self.DecompositionSet.append([x])
      else:
    ...
    for clus, nodes in clusters.items():
      ...
      if clus != currnode: #source node is not one of the clusters
        ...
        if DependencyMatrixRow not in self.DependencyMatrix:
          ...
          self.AllProbes.append(self.ProbeToClus + [self.probeno])
          self.probeno += 1
        ...
        if len(ds) > 1:
          for node in ds:
            if self.G.__contains__(int(node)):
              newprobe = nx.shortest_path(self.G,self.SourceNode,int(node))+[self.probeno]
...
def main():  
  ...
  filename = "Topologies/MinCutResults/GraphDepth2.txt"
  if os.path.exists(filename):
    os.remove(filename)
  with open(filename,'a') as fR:
    fR.write("Nodes\t"+"AvgDeg\t"+"TotPrbs\t"+"TPExecTime\t"+"MaxDepth\t"+"PCExecTime\t"+"No.ofPrbs\t"+"TotLenOfPrbs\t"+"PEExecTime\t"+"No.ofPrbs\t"+"TotLenOfPrbs\n")
  ...
  for nodes in range(10,101,10):
    for j in range(0,10):
      ...
      startnode = max([[clus, len(clusnodes)] for clus, clusnodes in clusters.items()],key=itemgetter(1))[0]     
      #print startnode
      
      obj = AllPaths(G,startnode)
      
      start = timeit.default_timer() 
      obj.compute_dependency_Matrix(startnode,clusters)    
      stop = timeit.default_timer()
      TPExecTime = str(stop - start)
  
    
      #for probe in obj.AllProbes:
        #print probe[:-1]
      ResultRow.append(float(sum(nx.degree(G).values()))/nodes) 
      ResultRow.append(TPExecTime)    
      ...
      #print colored("Total number of probes selected= %d" % obj.NumberOfProbesSelected, 'red')  
      #print colored("Total length of all probes = %d" % sum(f(lst) for lst in obj.ProbeSet), 'red')
