'''
Source Node is selected dynamically, the node which is at the shortest distance from all other nodes
...
class AllPaths:
  ...
  def ComputeDependencyMatrix(self):
    listofnodes = self.G.nodes() 
    listofnodes.remove(self.SourceNode)
    for endnode in range(0,self.num_of_nodes):    
    #for endnode in listofnodes:
      self.iterBFS(self.SourceNode,endnode)
